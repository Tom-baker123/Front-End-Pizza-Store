  ...
  return (
    <div>
      <div className="grid grid-cols-3 gap-5">
        {(size && size.length > 0) ?
          size.map((item, index) => (
            <div onClick={() =>  onSizeChange(item.additionalPrice)} className="flex flex-col items-center bg-green-50 gap-2 p-3 rounded-lg group cursor-pointer hover:bg-green-200" key={index} >
              {console.log("item", item.additionalPrice)}
